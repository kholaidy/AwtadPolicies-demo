...
(function(){
  ...
  function ensureStyles(){
    ...
    /* زر النسخ بشكل نصّي مع أيقونة */
    .awtad-ai .copy-btn{
      position:absolute;top:.5rem;left:.5rem;
      height:28px; padding:0 10px; width:auto;
      border:1px solid #4b5563;background:#1f2937;color:#e5e7eb;
      border-radius:6px;cursor:pointer;font-size:12px;
      display:inline-flex;align-items:center;gap:6px;
      opacity:.85;transition:opacity .2s,background .2s,color .2s,border-color .2s;
      white-space:nowrap;
    }
    .awtad-ai .copy-btn:hover{opacity:1;background:#374151}
    .awtad-ai .copy-btn .icon{font-size:14px;line-height:1}

    /* حالة النسخ الناجح */
    .awtad-ai .copy-btn.copied{background:#059669;color:#fff;border-color:#059669}
    ...
    /* تحسين السكروول بار */
    .awtad-ai .out{scrollbar-width:thin;scrollbar-color:#6b7280 transparent}
    .awtad-ai .out::-webkit-scrollbar{width:10px;height:10px}
    .awtad-ai .out::-webkit-scrollbar-track{background:#0b0f14;border-radius:8px}
    .awtad-ai .out::-webkit-scrollbar-thumb{background:#4b5563;border-radius:8px;border:2px solid #0b0f14}
    .awtad-ai .out:hover::-webkit-scrollbar-thumb{background:#6b7280}

    @media (prefers-color-scheme: light){
      :root{--c-bg:#fff;--c-fg:#111}
      .awtad-ai textarea{background:#fff;color:#111}
      .awtad-ai .out{background:#f9fafb;color:#111;scrollbar-color:#9ca3af transparent}
      .awtad-ai .copy-btn{background:#f3f4f6;color:#111;border-color:#d1d5db}
      .awtad-ai .copy-btn:hover{background:#e5e7eb}
      .awtad-ai .out::-webkit-scrollbar-track{background:#f3f4f6}
      .awtad-ai .out::-webkit-scrollbar-thumb{background:#9ca3af;border-color:#f3f4f6}
    }`;
  ...
  function openModal(){
    ...
    box.innerHTML=`
      ...
      <div class="body">
        ...
        <div id="awtad-ai-out" class="out">
          <button class="copy-btn" id="awtad-ai-copy-inline" title="نسخ الاستجابة">
            <span class="icon">📋</span><span class="label">Copy</span>
          </button>
        </div>
      </div>
      <footer>
        <button id="awtad-ai-send" title="إرسال" style="background:#1f2937; border:1px solid #4b5563; color:white; width:50px; height:40px; border-radius:8px; cursor:pointer; font-size:18px;">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: inline-block; vertical-align: middle; margin-top:-2px;"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>
        </button>
      </footer>`;
    ...
    // داخل function openModal() — تعريف دالة النسخ
    const copyResponse = (btn) => {
    // إنشاء نسخة من المحتوى وإزالة زر النسخ منها قبل أخذ النص
    const clone = out.cloneNode(true);
    clone.querySelector('#awtad-ai-copy-inline')?.remove();
    ...
    try{
        ...
        btn.innerHTML = `<span class="icon">✓</span><span class="label">Copied</span>`;
        btn.classList.add('copied');

        // المدة بالملّي ثانية (يمكنك تغييرها)
        setTimeout(()=>{
    ...
    // تحديث محتوى الخرج عند الضغط على الأزرار (يحافظ على زر النسخ بالنمط الجديد)
    box.querySelectorAll("[data-a]").forEach(btn=>{
      btn.addEventListener("click", async ()=>{
        ...
        if(action==="qa"){
          out.innerHTML = `
            <button class="copy-btn" id="awtad-ai-copy-inline" title="نسخ الاستجابة">
              <span class="icon">📋</span><span class="label">Copy code</span>
            </button>
            <div style="margin-bottom:.4rem;color:#b91c1c">تنبيه: الإجابات مقيدة بما ورد في السياسات فقط، وسيُعرض "غير مذكور في السياسات" إن لم يوجد نصّ مطابق.</div>
            جارٍ المعالجة…`;
        } else {
          out.innerHTML = `
            <button class="copy-btn" id="awtad-ai-copy-inline" title="نسخ الاستجابة">
              <span class="icon">📋</span><span class="label">Copy code</span>
            </button>جارٍ المعالجة…`;
        }
...
function handleSend() {
  ...
  // إن كان فارغًا، نستخدم نص الصفحة كمدخل للـ author (مع سؤال افتراضي للتصنيف)
  const pageText = (document.querySelector("main")?.innerText || document.body.innerText || "").slice(0,12000);
  const effectiveText = inputText || pageText;

  // نحفّظ آخر مدخل (سواء كان إدخال المستخدم أو نص الصفحة)
  try { localStorage.setItem("awtad_ai_buffer", effectiveText); } catch {}

  // عرض حالة المعالجة مع زر النسخ
  out.innerHTML = `
    <button class="copy-btn" id="awtad-ai-copy-inline" title="نسخ الاستجابة">
      <span class="icon">📋</span><span class="label">Copy code</span>
    </button>
  ...
  try {
    // نرسل 'auto' دائماً
    // - إذا كان هناك سؤال مكتوب: نمرره كما هو
    // - إن كان فارغًا: نولّد سؤالًا صغيرًا يطلب الاستشارة على نص الصفحة
    const questionText = inputText
      ? inputText.slice(0, 8000)
      : "حلّل نص السياسات المرفق وقدّم مخرجات استشارية موجزة ومباشرة.";
  
    const payload = {
      action: "auto",
      text: effectiveText.slice(0, 8000), // مصدر المحتوى (إدخال المستخدم أو نص الصفحة)
      question: questionText,             // سؤال المستخدم (أو سؤال افتراضي للتصنيف)
      page: location.pathname,
      urls: collectPolicyUrls()           // تمرير الروابط دائمًا
    };
    ...
    // تحديد تنسيق العرض بناءً على المخرجات
    if ((j.detected === "author") || (html.startsWith("<") && !html.includes("غير مذكور"))) {
      if(!/class=["']policy-content["']/.test(html)){ html = `<div class="policy-content">${html}</div>`; }
      out.innerHTML = `<button class="copy-btn" id="awtad-ai-copy-inline" title="نسخ الاستجابة">📋</button>` + html;
    } else {
      const txt = html.replace(/\n/g,'<br>');
      out.innerHTML = `<button class="copy-btn" id="awtad-ai-copy-inline" title="نسخ الاستجابة">📋</button>` + txt;
    }
...
// السماح بالإرسال عبر Enter (Shift+Enter لسطر جديد)
